...
当导入失败或结果不符合预期时，优先检查以下高频问题：

1. **颜色值范围** - RGB 是否在 0-1 范围（不是 0-255）
2. **填充对象属性** - `visible`/`opacity`/`blendMode` 是否错误地放在填充对象内
3. **VECTOR 节点** - 是否同时设置了 `fillGeometry` 和 `vectorPaths`
4. **Auto-Layout 配置** - `counterAxisAlignItems` 是否使用了无效的 `"STRETCH"`
5. **字体可用性** - 是否使用了未预加载的字体

完整的调试检查清单请参考 [generation-checklist.md](references/generation-checklist.md) 和 [faq-best-practices.md](references/faq-best-practices.md)。
...
| 问题类型 | 参考文档 |
|---------|---------|
| 常见错误和解决方案 | [faq-best-practices.md](references/faq-best-practices.md) |
| 节点类型和属性参考 | [figma-api-schema.md](references/figma-api-schema.md) |
| 矢量路径构造 | [vector-construction.md](references/vector-construction.md) |
| 复杂类型定义 | [complex-types.md](references/complex-types.md) |
| 生成前检查清单 | [generation-checklist.md](references/generation-checklist.md) |

### 推荐调试流程
1. 使用验证脚本：`python scripts/validate_json.py your-file.json`
2. 从最小示例开始，逐步添加属性
3. 每次添加后验证结果
4. 参考 FAQ 文档排查具体问题
